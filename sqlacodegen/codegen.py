...
def derive_model_class(existing_declarative_meta):
    table_model_mapping = {
        m.__tablename__: m.__name__
        for m in list(existing_declarative_meta._decl_class_registry.values())
        if hasattr(m, '__name__')
    }

    def tablename_to_classname(self, table_name, inflect_engine):
        return table_model_mapping.get(
            table_name, ModelClass._tablename_to_classname(table_name, inflect_engine)
        )

    return type(
        "ModelClass", (ModelClass,), {"_tablename_to_classname": tablename_to_classname}
    )
